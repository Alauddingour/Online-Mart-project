...
<header>
        <nav class="navbar-expand-md navbar-light fixed-top" id="navbar" >
          ...
          <div style="margin-top: 80px;">
           <h4>Results of search product</h4>
          <table class="table table-hover table-bordered" style="border: 1px solid black;margin-top: 20px">
				<thead>
					<tr>
						...
						<th>Brand Name</th>
						<th>Category Name</th>
						<th style="width: 200px">Product Added</th>
						<th style="width: 200px">Product Last Modified</th>
					</tr>
				</thead>
				<tbody>
						<tr>
							<td>${searchProduct.id}</td>
							<td style="max-width: 150px; 
                         word-wrap: break-word;overflow: hidden;
      text-overflow: ellipsis;">${searchProduct.name}</td>
							<td style="max-width: 150px; 
                         word-wrap: break-word;overflow: hidden;
      text-overflow: ellipsis;">${searchProduct.description}</td>
							<td>
          <img src="${pageContext.request.contextPath}/images/${product.image}" width="70" height="90" class="img img-responsiive" /></td>
          					<td class="text">${searchProduct.price}</td>
          					<c:if test= "${searchProduct.inStock==true}">
							<td class="text" style="color: green">Available</td>
							</c:if>
							<c:if test= "${searchProduct.inStock==false}">
							<td class="text" style="color: red">Unavailable</td>
							</c:if>
							<td class="text">${searchProduct.brands.name}</td>
							<td class="text">${searchProduct.category.name}</td>
							<td>${searchProduct.added_time}</td>
						    <td >${searchProduct.last_modified}</td>
						    <td style="padding-left: 10px"><a class="btn btn-secondary" href = "<spring:url value='/editProduct'/>?id=${searchProduct.id}">Edit</a></td>
						    <td style="padding-left: 10px"><a class="btn btn-secondary" style="background-color: red" href = "<spring:url value='/deleteProduct'/>?id=${searchProduct.id}">Delete</a></td>
						    <c:if test= "${searchProduct.inStock==true}">
						    <td style="padding-left: 10px"><a class="btn btn-secondary" href = "<spring:url value='/changeAvailability'/>?id=${searchProduct.id}">Make Unavailable</a></td>
						    </c:if>
						    <c:if test= "${searchProduct.inStock==false}">
						    <td style="padding-left: 10px"><a class="btn btn-secondary" href = "<spring:url value='/changeAvailability'/>?id=${product.id}">Make Available</a></td>
						    </c:if>   
						</tr>
				</tbody>
			</table>
