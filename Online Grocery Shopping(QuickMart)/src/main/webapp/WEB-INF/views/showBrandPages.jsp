     ...
     <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
      ...
      $(function()
    	  ...
    	  $("#searchButton").click(function()
    			  {
    		     $('#brandSearch').find('option').each(function() {
      		               if($(this).text()==$("#browser").val())
      		            	   {
      		            	     var id= $(this).data("value");
      		            	   window.location="<spring:url value='/searchBrand'/>?id="+id;
      		            	   }
      		          });
    			  })
...
<input  list="brand" name="brand" id="browser" placeholder="Search Brands..." style="width: 400px;border-radius: 8px" list="category"> 
 <datalist id="brand">
 <select id="brandSearch">
    <c:forEach items="${allBrands}" var="brand">
        <option data-value="${brand.id}" value="${brand.name}">${brand.name}</option>
    </c:forEach>
</select>
  </datalist>
  ...
  ${message}
	...
	<table class="table table-hover table-bordered" style="border: 1px solid black;margin-top: 20px">
      ...
      text-overflow: ellipsis;">${brand.name}</td>
						    <td style="padding-left: 10px;width: 100px"><a class="btn btn-secondary" href = "<spring:url value='/editBrand'/>?id=${brand.id}">Edit</a></td>
						    <td style="padding-left: 10px;width: 100px"><a class="btn btn-secondary" style="background-color: red" href = "<spring:url value='/deleteBrand'/>?id=${brand.id}">Delete</a></td>
						</tr>
					</c:forEach>
					</c:when>
					<c:otherwise>
        ...
        <div class="panel-footer">
            <ul class="pagination">
                <c:forEach begin="0" end="${totalPages-1}" var="page">
                    <li class="page-item">
                        <a href="showBrandPages?pageNum=${page+1}" class="page-link">${page+1}</a>
                    </li>
                </c:forEach>
            </ul>
        </div>
